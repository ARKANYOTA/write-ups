# from myhdl import *
from myhdl import block, always_comb, Signal, always, modbv, now, delay
...
@block
def Block0(a, z):
    # Bin le premier bin des 17 lettre de l'input
    b = [Signal(bool(a[i])) for i in range(len(a))]
    b = [Signal(bool(i)) for i in [True, False, False, False, True, False, False, True, False, False, True, True, False, False, True, False, True]]
    t = [Signal(bool(0)) for _ in range(56)]
    F0 = NOT(b[8], t[0])
    F1 = AND(t[0], b[4], t[1])
    F2 = NOT(b[13], t[2])
    F3 = NOT(b[1], t[3])
    ...
    F42 = NOT(b[10], t[42])
    F43 = OR(t[42], b[3], t[43])
    F55 = and(t[18], not(and(t[31], not(and(t[34], not(and(or(t[40], and(t[41], t[43])), not( and(b[4], and(b[8], b[13])) ))))))))
"""

    @always_comb
    def core():
        z.next = Signal(True)   # t[55]
...
@block
def Tulipe(go_in, little_box, undeuxtroisetplusencore): # go_in, little_box, undeuxtroisetplusencore
    @always_comb
    def core():
        if undeuxtroisetplusencore == 0:
            little_box.next = go_in[0]
        elif undeuxtroisetplusencore == 1:
            little_box.next = go_in[1]
        elif undeuxtroisetplusencore == 2:
            little_box.next = go_in[2]
        elif undeuxtroisetplusencore == 3:
            little_box.next = go_in[3]
        elif undeuxtroisetplusencore == 4:
            little_box.next = go_in[4]
        elif undeuxtroisetplusencore == 5:
            little_box.next = go_in[5]
        elif undeuxtroisetplusencore == 6:
            little_box.next = go_in[6]
        elif undeuxtroisetplusencore == 7:
            little_box.next = go_in[7]
...
def plic_ploc(input, output, sel, clk):
    @always(clk.posedge)
    def logic():
        print("plic_ploc", input, output, sel)
...
def Silicon(Flag_in):
    ...
    """
    Flag_in[0] = modbv(70245, 0, 2**17)
    Flag_in[1] = modbv(253902, 0, 2**17)
    Flag_in[2] = modbv(70245, 0, 2**17)
    Flag_in[3] = modbv(70245, 0, 2**17)
    Flag_in[4] = modbv(70245, 0, 2**17)
    Flag_in[5] = modbv(70245, 0, 2**17)
    Flag_in[6] = modbv(70245, 0, 2**17)
    Flag_in[7] = modbv(70245, 0, 2**17)
    """
    ...
    print(go_in, little_box, undeuxtroisetplusencore)
...
if len(attempt) == 17:
    ...
    for l in attempt:
        ...
        print("\tletter:", bin(letter))
        for i in range(8):
            bus[i] += letter[i] * 2 ** x

            print("\t\ti, bus[i], letter[i] * 2 ** x", i, bus[i], letter[i] * 2 ** x)
        x += 1

arr = [Signal(line) for line in bus]
simInst = Silicon(arr)
simInst.run_sim(170)
