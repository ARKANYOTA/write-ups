...
@block
def Senghor(buron, larbaud, leroux, pisan, pascal, bel, schmitt, sagan, signol, torpor, verdier, tocqueville, tuffrau, quinet):
    #       buron, gottis, chamfort, marot, coco, gaboriau, beda, bert, grandbois, hallier, deleuze, diderot, cloux, bordes
    @always_comb
    def core():
        if leroux == 1:
            if buron == 0x4 and larbaud == 0x18:
                sagan.next = 1
                pisan.next = 0
                pascal.next = 0
                bel.next = 0
                schmitt.next = 0
            elif buron != 0x00 or larbaud != 0x00:
                levy = buron >> 1
                if levy in (
                    ...
                    if levy in (0x3, 0x6a):
                        ...
                        signol.next = 1
                pascal.next = ((buron & 0b1) << 2) | (larbaud >> 6)
